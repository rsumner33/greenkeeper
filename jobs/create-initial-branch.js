...
>>>>>>>+HEAD
      aw      await upsert(repositories, repository._id, { enabled: true })
      return await maybeUpdatePaymentsJob(accountId, repository.private)
    } else {
>>>>>>>-8be4bfc
row new Error('Could not create initial branch')
    }
  }
...
>>>>>>>+HEAD
  const   const depsUpdated = transforms[0].created
  const travisModified = transforms[1].created
  const badgeAdded = transforms[2].created

>>>>>>>-8be4bfc
upsert(repositories, `${repositoryId}:branch:${sha}`, {
    type: 'branch',
    initial: true,
    sha,
    base: branch,
...
>>>>>>>+HEAD
async fuasync function createDefaultLabel ({github, name, owner, repo}) {
  if (name !== false) {
    try {
